...
dpt_get_raw(void)
{
        ...
        g_dpt_raw.inner = (u32)LF(DPT_BITS) - ((u32)g_dpt_rx_buf[5] << 16 | g_dpt_rx_buf[4] << 8 | g_dpt_rx_buf[3]);
        g_dpt_raw.outer = (u32)LF(DPT_BITS) - ((u32)g_dpt_rx_buf[8] << 16 | g_dpt_rx_buf[7] << 8 | g_dpt_rx_buf[6]);
        return g_dpt_raw;
}
...
{
        g_dpt_outer_theta = ((f32)dpt_get_raw().outer / (f32)LF(DPT_BITS)) * TAU;
