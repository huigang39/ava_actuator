...
dpt_get_raw(void)
{
        ...
        g_dpt_raw.inner  = (u32)BIT(DPT_BITS) - ((u32)g_dpt_rx_buf[2] << 16 | g_dpt_rx_buf[1] << 8 | g_dpt_rx_buf[0]);
        g_dpt_raw.outer  = (u32)BIT(DPT_BITS) - ((u32)g_dpt_rx_buf[5] << 16 | g_dpt_rx_buf[4] << 8 | g_dpt_rx_buf[3]);
...
{
        g_dpt_inner_theta = ((f32)g_dpt_raw.inner / (f32)BIT(DPT_BITS)) * TAU;
...
{
        dpt_get_raw();
        g_dpt_outer_theta = ((f32)g_dpt_raw.outer / (f32)BIT(DPT_BITS)) * TAU;
